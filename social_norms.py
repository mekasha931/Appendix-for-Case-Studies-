...
This simulation models how a social meme spreads across a population of interacting agents
and matures into a stable social norm once the collective internalization (M(t)) crosses a critical threshold E_c.
...
lambda_decay = 0.1           # Memory decay rate (exponential kernel)
E_c = 0.6                    # Emergence threshold for social norm
noise_level = 0.02           # Magnitude of contextual noise (random shocks)
...
interaction_matrix /= interaction_matrix.sum(axis=1, keepdims=True)

# Initialize kernel and history of system-wide meme strength
kernel = lambda dt: np.exp(-lambda_decay * dt)
M_t = []  # M(t): Mean meme strength over time
...
plt.subplot(1, 3, 1)
plt.plot(range(T), M_t, label='M(t)', color='navy')
plt.axhline(y=E_c, color='red', linestyle='--', label='Emergence Threshold $E_c$')
plt.xlabel("Time")
plt.ylabel("Global Meme Strength $M(t)$")
plt.title("Emergence of Social Norm")
plt.legend()

# Plot 2: Heatmap of meme strength at final time step
plt.subplot(1, 3, 2)
plt.imshow([meme_strength], cmap='viridis', aspect='auto')
plt.colorbar(label='Meme Strength')
plt.title("Final Meme Strength Distribution")
plt.yticks([])

# Plot 3: Final average vs. threshold
plt.subplot(1, 3, 3)
plt.plot(range(T), [np.mean(meme_strength)] * T, color='purple', label='Final Mean')
plt.axhline(y=E_c, color='red', linestyle='--')
plt.title("Final System State vs Threshold")
plt.xlabel("Time")
